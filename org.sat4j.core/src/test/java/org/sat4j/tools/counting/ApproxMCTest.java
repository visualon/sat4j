...
public class ApproxMCTest {

    /**
     * The tolerance to use for the tests.
     */
    private static final double EPSILON_TEST = .5;

    /**
     * The confidence to use for the tests.
     */
    private static final double DELTA_TEST = .5;
    ...
    /**
     * Test method for {@link org.sat4j.tools.ApproxMC#countSolutions()}.
     */
    @Test
    public void testCountSolutions() {
        int nbCorrectApprox = checkModelCount();
        assertTrue(nbCorrectApprox + " < " + EXPECTED,
                nbCorrectApprox >= EXPECTED);
    }

    /**
     * Checks that {@link ApproxMC} finds an estimate count within the
     * theoretical bounds with high probability on the given input.
     * 
     * @param solver
     *            The input solver.
     * @param realCount
    ...
    private int checkModelCount() {
        ...
        ApproxMC counter = new ApproxMC(solver, EPSILON_TEST, DELTA_TEST);
        for (int i = 0; i < NUMBER_OF_TRIES; i++) {
            BigDecimal nbSolutions = new BigDecimal(counter.countSolutions());
